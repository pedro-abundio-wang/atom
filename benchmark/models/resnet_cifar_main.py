...
from benchmark.models import resnet_cifar_model, resnet_common
...
def learning_rate_schedule(current_epoch,
    ...
    initial_learning_rate = resnet_common.BASE_LEARNING_RATE * batch_size / 128
...
def run(flags_obj):
    ...
    steps_per_epoch = (
            cifar10_preprocessing.NUM_IMAGES['train'] // flags_obj.batch_size)
    lr_schedule = 0.1
    if flags_obj.use_tensor_lr:
        ...
        lr_schedule = tf.keras.optimizers.schedules.PiecewiseConstantDecay(
            ...
            values=[initial_learning_rate] + list(p[0] * initial_learning_rate for p in LR_SCHEDULE))

    with strategy_scope:
        optimizer = resnet_common.get_optimizer(lr_schedule)
        model = resnet_cifar_model.resnet56(classes=cifar10_preprocessing.NUM_CLASSES)
        model.compile(
